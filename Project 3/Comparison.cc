...
OrderMaker :: OrderMaker(std::ifstream &myfile) {
	numAtts = 0;
	string types;
	while(myfile.good()){
		getline (myfile,types);
		whichAtts[numAtts] = atoi(types.c_str());
		getline (myfile,types);
		if(types!=""){
			if (types == "Int")
				whichTypes[numAtts] = Int;
			else if (types == "Double")			// now add in the doubles
				whichTypes[numAtts] = Double;
			else if (types == "String")				// and finally the strings
				whichTypes[numAtts] = String;
			else
					cout  << types << "meta file has been tampered please try by creating again\n";
			//cout << "now type is --------------------------------- " << types <<endl;
			numAtts++;
		}
...
void OrderMaker :: Printfile (std::ofstream &myfile) {
//	myfile << "NumAtts = " << numAtts << "\n";
	for (int i = 0; i < numAtts; i++)
	{
		myfile << whichAtts[i] << endl ;
		if (whichTypes[i] == Int)
		myfile << "Int\n";
		else if (whichTypes[i] == Double)
		myfile << "Double\n";
...
void  OrderMaker::qoMaker(OrderMaker* sortOrder, const CNF& query,
                                 OrderMaker& qorder, OrderMaker& qorder1){
  qorder.numAtts = qorder1.numAtts = 0;
  int attr;
	int lit;
  int found = 0;
  for(int j=0;j<sortOrder->numAtts;j++){
	 // cout << "now j is --------------- "<< j  <<endl;
	  for(int i=0;i<query.numAnds;i++){
		//cl  cout << "now i is --------------- "<< i  <<endl;
		  if(query.orLens[i]==1){
			  const Comparison& cmp = query.orList[i][0];
			  if ((cmp.op == Equals) && ( ( (cmp.whichAtt1==sortOrder->whichAtts[j]) && (cmp.operand2==Literal) )
              						||( (cmp.whichAtt2==sortOrder->whichAtts[j]) && (cmp.operand1==Literal) ) ) ){
										  attr = cmp.whichAtt1;
					lit = cmp.whichAtt2;
					if (cmp.operand1 == Literal)
					{
						lit = cmp.whichAtt1;
						attr = cmp.whichAtt2;
					}
										  qorder.whichAtts[qorder.numAtts]=attr;
										  qorder.whichTypes[qorder.numAtts]=sortOrder->whichTypes[j];
										  qorder1.whichAtts[qorder1.numAtts] = lit;
										  qorder1.whichTypes[qorder1.numAtts] = sortOrder->whichTypes[j];
										  ++qorder.numAtts, ++qorder1.numAtts;
										  found = 1;
									  }
  ...
  cout << " matched shit is " << qorder.numAtts << endl;
}

OrderMaker* CNF::CnfqoMaker(OrderMaker &srtorder){

    OrderMaker cnf_order;
    OrderMaker sortOrderCopy = srtorder;
    GetSortOrders(cnf_order, sortOrderCopy);
	...
	cout << "matched shit in new " << query->numAtts << endl;
    if(query->numAtts > 0)
        return query;
    else
    {
